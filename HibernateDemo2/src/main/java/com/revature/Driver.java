...
public class Driver {
	...
	public static void main(String[] args) {
		...
		System.out.println("GETTING ALL FLASHCARDS!");
		// SELECT ALL FLASHCARDS
		List<?> target = s.createNamedQuery("getAllPokemon").getResultList();
		for(int i = 0; i<target.size();i++) {
			System.out.println(target.get(i));
		}
		System.out.println("----------------------------------------------------------------------------------------------------------");
		System.out.println("GETTING ALL POKEMON FLASHCARDS, NOT NATIVE");
		// THIS IS A NAMED NOT NATIVE QUERY
		List<?> target3 = s.createNamedQuery("thatPokemon").setParameter("var1", "who's that Pokemon?").getResultList();
		for(int i = 0; i<target3.size();i++) {
			System.out.println(target3.get(i));
		}		
		System.out.println("----------------------------------------------------------------------------------------------------------");
		System.out.println("GETTING ALL POKEMON FLASHCARDS, NATIVE");
		// THIS IS A NAMED NATIVE QUERY		
		List<?> target2 = s.createNamedQuery("thatPokemonNative").setParameter("var2", "who's that Pokemon?").getResultList();
		...
		/*
		 * 
		 * FlashcardDAO fd = new FlashcardDAOImpl(); fd.addFlashcard(new
		 * Flashcard("who's that Pokemon?", "it's Squirtle", null)); fd.addFlashcard(new
		 * Flashcard("who's that Pokemon?", "it's not Bulbasaur", null));
		 * fd.addFlashcard(new Flashcard("who's that Pokemon?",
		 * "it's definitely Bulbasaur", null)); fd.addFlashcard(new
		 * Flashcard("who's that Pokemon?", "it's Pikachu", null)); fd.addFlashcard(new
		 * Flashcard("who's that Pokemon?", "it's Blue-Eyes-White-Dragon", null));
		 */
	...
	public static void funWithUpdateAndMerge(SessionFactory sf) {
		...
		Flashcard f2 = new Flashcard(23, "what else is Java?", "an acronym for Just A Valid Answer",
				new Topic(1, "coding"));
		...
		// merge: will check whether a persistent version exists, then copy over values
		// onto persistent obj
		s1.merge(f2);
	...
	public static void funWithGetAndLoad(SessionFactory sf) {
		...
		// System.out.println(f3); // forcing lazily fetched object to be retrieved by
		// accessing it
